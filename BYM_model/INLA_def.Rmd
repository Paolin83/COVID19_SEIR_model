...
```{r}
## include pop e hubei
formula.int_n<-  totale_casi ~ 1+log(hubei+1)+log(pop)+ f(ID,model="bym",graph=file.adj) +
f(t,model="rw2") + f(t2,model="iid") +
f(t3,model="iid", group=ID2, control.group=list(model="besag", graph=file.adj))


fit_n<-inla(formula.int_n, family="poisson", data=dat_csv_n,control.compute = list(dic=T),control.predictor = list(link = 1))
summary(fit_n)
```
...
st<-fit_n$summary.random$t3$mean
## Trends for province
