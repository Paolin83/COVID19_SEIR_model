...
We predict beta (and R0) for the next 14 days by means of a linear regressione model, assuming a Log-normal distribution for the beta (the slope) forcing its value to be greater than 0.
```{r R0 forecast}

time<-3:(days-2)
beta.model<-glm(log(beta_vec)~time,weights = 1/sd_vec,family=gaussian)
forecast=14
...
beta.predict[,3:5]<-exp(predict[,3:5])
...
We made a forecast forward of 14 days.  
...
We save the results and plot a credible interval at 90% (5-95% percentile).
...
p <- dygraph(mu,main=paste("Italy: Scenario 1 Credible (confidence ",100*0.90,"%)",sep = ""),ylab=" Infected",xlab="Day",height=400,width=800) %>%  dySeries(c("mu.lower", "counts", "mu.upper"),label="counts")
...
counts<-mu.med
mu<-xts(x = as.matrix(cbind(counts,mu.lower,mu.upper)) , order.by = c(days.before,days.ahead))
p <- dygraph(mu,main=paste("Italy: Scenario 2 Credible (confidence ",100*0.90,"%)",sep = ""),ylab=" Infected",xlab="Day",height=400,width=800) %>%  dySeries(c("mu.lower", "counts", "mu.upper"),label="counts")
p<-p %>% dyLegend(show = "always", hideOnMouseOut = FALSE) %>%  dyShading(from = days.ahead[1], to = days.ahead[step.ahead], color = "#CCEBD6")%>% dyEvent(days.ahead[1], "Prediction", labelLoc = "bottom")
p

### confidence limits - scenario 3
I_seir_med<-tapply(seir3_sim$I,seir3_sim$time,median)
I_seir_lwr<-tapply(seir3_sim$I,seir3_sim$time,quantile,p=0.05)
I_seir_upr<-tapply(seir3_sim$I,seir3_sim$time,quantile,p=0.95)
...
counts<-mu.med
mu<-xts(x = as.matrix(cbind(counts,mu.lower,mu.upper)) , order.by = c(days.before,days.ahead))
p <- dygraph(mu,main=paste("Italy: Scenario 3 Credible (confidence ",100*0.90,"%)",sep = ""),ylab=" Infected",xlab="Day",height=400,width=800) %>%  dySeries(c("mu.lower", "counts", "mu.upper"),label="counts")
p<-p %>% dyLegend(show = "always", hideOnMouseOut = FALSE) %>%  dyShading(from = days.ahead[1], to = days.ahead[step.ahead], color = "#CCEBD6")%>% dyEvent(days.ahead[1], "Prediction", labelLoc = "bottom")
p
```

The 3 scenarios show different numbers. 
If we consider the second scenario, at the end of the 2 weeks (`r date[length(date)]`) the number of infected is  (`r (tapply(seir2_sim$I,seir2_sim$time,median)*N)[forecast]`).  

In the next plot the cumulative number of infected.  
At the end of the 2 weeks (`r date[length(date)]`) the total number of COVID19 cases is expected to be  (`r (tapply(seir2_sim$I,seir2_sim$time,median)*N+tapply(seir2_sim$R,seir2_sim$time,median)*N)[forecast]`).  
